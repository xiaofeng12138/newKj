...
export default {
    ...
    created(){
        ...
        if(this.billNo){
            let r = [];
            //当角色是报关行的时候，调用接口不同a
            if(this.$route.params.role&&this.$route.params.role.indexOf('CB')!=-1){
                this.CNCOMPANYCODE = this.$route.params.CNCOMPANYCODE;
                 if(this.$route.params.erpTempnum){
                     r = publicInter(interfaceUrl.customsBrokerMergerBLLinkWithOrderAfChaiDan,{
                         "billNo":this.billNo,
                     ...
                     r = publicInter(interfaceUrl.customsBrokerMergerBLLinkWithOrder,{'transmode':this.transmode,"billNo":this.billNo,'CNCOMPANYCODE':this.CNCOMPANYCODE})
                 }
                 ...
                 //企业拆单状态
                if(this.$route.params.erpTempnum){
                    r = publicInter(interfaceUrl.getChaiDanHouOneTicBlNoMateAddMateRelaAfMer,{
                        "billNo":this.billNo,
                        erpTempnum:this.$route.params.erpTempnum
                    })
                }
                //企业常规状态
                    ...
                    if(this.$route.params.air=='yes'){
                        r = publicInter(interfaceUrl.POListAfMerger,{"billNo":this.billNo,air:'yes'})
                    }else{
                         r = publicInter(interfaceUrl.POListAfMerger,{"billNo":this.billNo})
                    }
            ...
            r.then(r=>{
                if(r['code']=='200'){
                    ...
                    this.$Modal.confirm({
                        title: '',
                        content: '<p>'+r.errMsg+'</p><br/><br/><p>是否立即进行物料主数据上传？</p>',
                        onOk: () => {
                            this.$router.push({
    ...
    methods:{
        ...
        preConfirm(){
            if(this.msg.message){
                ...
                this.$Modal.confirm({
                    ...
                    onOk:()=>{
                        that.$router.push({name:'ERPInformationUpload',params:{name:'name4'}})
        ...
        confirm(){
            ...
            let url = (this.role&&this.role.indexOf('CB')!=-1)?interfaceUrl.MaterialListcb:interfaceUrl.MaterialList;
            //拆单的参数，和拆单时的url
            if(this.$route.params.erpTempnum){
                url = this.role&&this.role.indexOf('CB')!=-1?interfaceUrl.addMergeredMaterialListAfChaiDan:interfaceUrl.addMergeredMaterialListAfChaiDan2;
            ...
            publicInter(url,params).then(r=>{
                if(r.code=='200'){
                    ...
                    if(this.CNCOMPANYCODE !== ""){
                        if(this.$route.params.erpTempnum){
                            this.$router.push({name:'sort',params:{
                                billNo:this.billNo,
                                transmode:this.transmode,
                                erpTempnum:this.$route.params.erpTempnum,
                                CNCOMPANYCODE:this.CNCOMPANYCODE,
                            ...
                            this.$router.push({name:'sort',params:{billNo:this.billNo,CNCOMPANYCODE:this.CNCOMPANYCODE,role: "ROLE_CB",transmode:this.transmode}})
                        }
                    }
                    else{
                        if(this.$route.params.erpTempnum){
                            ...
                            this.$router.push({name:'sort',params:{billNo:this.billNo,transmode:this.transmode}})
                        }                       
                    }
