<template>
  <div class="agreements" >
       ...
       <Select v-model="valueSelect" style="width:15%;margin-right:30px;margin-top:1.5px" size="large">
         ...
         <Input v-model="form.agreementId" size="large" placeholder="请输入协议编号" clearable style="width: 25%;margin-right:30px"></Input>
        <div class="btnBox">
        ...
        <!-- <detail ref="selfModal" :showField='filed' :title="'框架协议详情'"></detail> -->
  </div>
</template>
<script>
...
export default {
        ...
        return{
            filed:{
                head:[
                    ...
                    {key:'CNCompanyEmailAdress',value:'中方公司电子邮箱'},
                        ...
                        render: (h, params) => {
                            ...
                            return h('div', [
                                  h('Button',{
                                    ...
                                    on:{
                                        click(){
                                            that.getdetail(params.row.AGREEMENTID)
                                        }
                                    }
                                },'查看'),
                                  h(Tablepdf,{
                                ...
                                h('Poptip', {
                                    ...
                                    nativeOn:{
                                       click(e){
                                           ...
                                           if(text==='确定'||text==='ok'){
                                               that.cancel(params.row.AGREEMENTID,params.index)
                                           }
                                       }
                                    }
                                },[
    ...
    methods:{
        ...
        getLists(){
            publicInter(interfaceUrl.AgreementList,this.form).then(r=>{
                if(r){
                     r.list.forEach(item => {
        ...
        getdetail(id){
            publicInter(interfaceUrl.AgreementDetail,{agreementId:id}).then(r=>{
                if(r){
                    this.$emit('showDetail',{title:'框架协议详情',data:r,key:this.filed})
        ...
        cancel(id,index){
            publicInter(interfaceUrl.delAgreement,{agreementId:id}).then(r=>{
                if(r&&r.code==='200'){
