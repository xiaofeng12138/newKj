<template>
    <div class="tagsWrap">
        <Row style="border-bottom:2px solid #ccc;margin-bottom:20px !important">
            <Col span="12"><h2>授权企业分类查询</h2></Col>
            <Col span="12">
                    <div  style="float:right;"><h2>公司名称:{{showCompanyName}}</h2></div>
            </Col>
        </Row>
        <div class="noTags" v-if="queryTagsList.length == 0">暂无标签信息</div>
        <Tabs class="tabsWrap"  @on-click='clickTab' v-else>
             <TabPane v-for="(item,index) in queryTagsList " :key = index :label='item.lablename'>
                 <div class="query" >
                    ...
                    <Button type='primary'  @click="openNewModal" style="margin-right:10px;width:100px">新  增</Button>
                ...
                <div style='margin-top:20px'>
                    <Table border :columns = 'columns' :data='tagsList' max-height='600' class="self" @on-selection-change= 'selectAll'>
        ...
        <Modal
         ...
         <p slot="header" style="text-align:center;font-size:15px">
            ...
            <Row >
                <Col span="12"><FormItem label='请选择标签'  prop='lablename'> 
                    <Select v-model="formData.lablename"  placeholder="请选择标签名称" disabled  >
        ...
        <Modal
         ...
         <!-- 批量导入 -->
        <allUpload ref="allUpload" :labelName = nowChooseTags />
    </div>
</template>
<script>
...
export default {
    ...
    methods:{
        showUploadModal(){
         ...
         showUploadOne(){
           this.formData.lablename = this.nowChooseTags
           this.addModal = true
        ...
        queryAllInfo(page){
         ...
         //点击Tab
        clickTab(params){
            let value = this.queryTagsList[params].lablename
