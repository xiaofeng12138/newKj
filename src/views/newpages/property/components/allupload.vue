    ...
    <div>
         <Modal
                ...
                <h3>
                    ...
                    <div class="selectWrap" v-if="!uploadLabelName"> 
                        <span>权利人名称</span>
                        <Select v-model="chooseLable"  placeholder="请选择权利人名称">
...
export default {
    ...
    methods:{
        ...
        upload(){
            ...
            }else if( !this.uploadLabelName && !this.chooseLable){
                 this.$Message.error({
                        content:"权利人名称不能为空",
                        duration:3
                    })
            ...
            var url = interfaceUrl.excelImport //文件上传地址
            this.filelist.forEach(item=>{
                let labelName = this.uploadLabelName ? this.uploadLabelName :this.chooseLable
                formdata.append('file',item)
                formdata.append('labelName',labelName)
            })
            fileUp(url,formdata).then(r=>{
                if(r.code == 200){
                    this.chooseLable.length > 0 ? this.$parent.queryTagList(1)  : ''
                    this.newLabel.length > 0 ? this.$parent.queryTagList(1)  : ''
