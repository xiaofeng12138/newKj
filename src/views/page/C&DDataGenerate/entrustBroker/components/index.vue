...
export default {
  ...
  props: {
    ...
    queryUrl: {
      type: String,
      required: false,
      default: interfaceUrl.queryBLList
    },
    judgeUrl: {
      type: String,
      required: false,
      default: interfaceUrl.changeBLToCustomsBrokerStatus
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    query() {
      ...
      //合并参数（空运多一个参数）
      params = { ...params, ...this.paramsAir };
      publicInter(this.queryUrl, params).then(r => {
        if (r) {
          this.data1 = [];
          r = r.result;
          for (var i = 0; i < r.length; i++) {
            if (
              !r[i].allBLListMap.ISENTRUST &&
              (r[i].allBLListMap.ACTION == "I1" ||
                r[i].allBLListMap.ACTION == "I2")
            ) {
            } else {
    ...
    unBatchEntrustFun(paramsArray) {
      ...
      for (var i = 0; i < selection.length; i++) {
        billNos.push({
          ...
          BECOMMEDCUSTOMSBROKER: selection[i].BECOMMEDCUSTOMSBROKER
        });
      }
      var params = {
        billNos: billNos,
      ...
      params = { ...params, ...this.paramsAir };
      // let url = this.role==='ROLE_CB'?interfaceUrl.changeBLToCustomsBrokerStatuscb:interfaceUrl.changeBLToCustomsBrokerStatus;
      publicInter(this.judgeUrl, params).then(r => {
        if (r) {
          if (r.code == "200") {
            this.modal1 = false;
            this.$Message.info({
