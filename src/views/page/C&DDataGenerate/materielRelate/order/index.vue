...
export default {
              ...
              h(
                "Button",
                {
                  props: {
                    ...
                    size: "small",
                    disabled:
                      this.status === "已关联" || this.dingdan === "no"
                        ? true
                        : false
                  },
                  style: {
  ...
  methods: {
    ...
    materialClick(type) {
      ...
      if (
        !this.orderIndex >= 0 &&
        this.cachedModifyValue > 0 &&
        !isNaN(this.cachedModifyValue)
      ) {
        if (type === "count") {
    ...
    async guibing() {
      if (this.$route.params.air == "yes") {
        var res = await this.submitMergeRelationShip({
          billNo: this.billNo,
          air: "yes"
        });
      } else {
      ...
      switch (type) {
        ...
        case "success":
          ...
          if (this.$route.params.air == "yes") {
            _this.$router.push({
              name: "result",
              params: { billNo: _this.billNo, air: "yes" }
            });
          } else {
            _this.$router.push({
              name: "result",
              params: { billNo: _this.billNo }
            });
          }
  ...
  computed: {
    ...
    ...mapGetters("bill", ["pureOrderList", "pureOrderListYGL", "confirmMsg"])
  },
  mounted() {
