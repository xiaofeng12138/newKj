...
import TablePath from "./components/path.vue";
...
export default {
  ...
  methods: {
    ...
    exportFun() {
      ...
      publicInter(interfaceUrl.donlowDataList, { billCode: this.value }).then(
        res => {
          if (res) {
            if (res.code === "500" || res.code === "400") {
              this.$Message.error(res.msg);
            } else {
              filedownload(interfaceUrl.donlowDataList, {
                billCode: this.value
              }).then(r => {
