<template>
    <div class="call-record">
        ...
        <Modal v-model="modalShow" width="1600" class="consult-modal" :footer-hide="hideFooter">
            ...
            <div class="modal-content">
                <Row class="modal-row">
                    ...
                    <Col :span="2" class="col-name">咨询时间</Col>
                    <Col :span="5">{{detailData.UPTIME}}</Col>
                </Row>
                <div>
                    <p class="answer-desc">{{detailData.DES}}</p>
                ...
                <div v-else>   
                    <div class="answer-msg">
                        ...
                        <div style="margin-left: 10px">
                            ...
                            <span class="answer-time">{{detailData.ANSTIME}}</span>
                        </div>
                    </div>
                    <p class="answer-desc">{{detailData.ANSWERDES}}</p>
                    <img style="width: 200px; height: 200px;margin-left: 10px" v-for="(item, index) in detailData.ansFiles" :key="index" :src="`/video/${item}`">
                </div>
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: (h, params) => {
                        return h('div', [
                            h('span', params.row.DES.slice(0, 20)),
                            h('span',{
                                style: { 
                                    cursor: 'pointer',
                                    color: '#fbd500'
                                ...
                                on: {
                                    'click': () => {
                                        ...
                                        if(this.detailData.ANSFILENAME != ''){
                                            this.detailData.ansFiles = this.detailData.ANSFILENAME.split(',')
                    ...
                    render:(h, params) => {
                        return h('span', {
                            style: {
                                width: '100%',
                                height: '100%',
                                color: params.row.STATUS == '0' ? '#fbd500' : '#fff'
                            },
                            on: {
                                'click': () => {
                                    this.detailData = params.row;
    ...
    methods: {
        ...
        query() {
            publicInter(interfaceUrl.queryScreenMsg, this.formData).then( r => {
                if(r.code == 200) {
        ...
        submit() {
            this.formData.PAVILION = this.pavilion;
