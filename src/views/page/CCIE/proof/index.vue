<template>
    <div class="exhibitorSearch"  style="text-align:left">
         <Row style="margin-bottom:10px; text-align:left;">       
            <!-- <span style="font-size: 13px">报关单号:</span> -->
            <Input v-model="certificateno" placeholder='请输入报关单号'  size='large' style="width:150px;margin-right:5px" class='customInput'/>
            <!-- <span style="font-size: 13px">提运单号:</span> -->
              <Input v-model="billno"  placeholder='请输入提运单号' size='large' style="width:150px;margin-right:5px" class='customInput'/>
            <!-- getCounInfoByParam -->
             <Input v-model="agentname" size='large' placeholder='请输入代理报关机构' style="width:150px;margin-right:10px" class='customInput' />
             <DatePicker type="date" class="customDatePicker"  placeholder="起始日期" size='large' style="width: 150px;margin-right:10px" v-model="startDate"></DatePicker>
            <DatePicker type="date" class="customDatePicker" placeholder="结束日期" size='large' style="width: 150px;margin-right:10px" v-model="endDate"></DatePicker>
            <Select v-model="valueSelect" class="customSelect" style="width:150px" size='large' placeholder="请选择审批状态">
            ...
            <div style="width: 100px;display: inline-block;margin-left: 20px">
                <Button type="primary" size='large' long @click="approvalPending">待审批</Button>
            </div>
            <div style="width: 100px;display: inline-block;margin-left: 20px">
                <Button type="primary" size='large' long @click="agreed">已同意</Button>
            </div>
        </Row>
        ...
        <div style="padding-top: 20px;">
            <Table :data="tableData1" :columns="tableColumns1" @on-selection-change='changeHandler' stripe size='large' class="customTable"></Table>
            <div style="margin: 10px;overflow: hidden;text-align:center" >
                    <Page :total="totalNumber" class="customPage"  :current="form.pageNum" @on-change="changePage" show-total></Page>
            </div>
        </div>
...
export default {
          ...
          render:(h, params)=>{
              return h('span',{
                 style:{
                  ...
                  on:{
                    click:()=>{
                      let id = params.row.BILLNO
                      this.$router.push({name:'jbhoverView',params:{
                        number:params.row.BILLNO,
                    }})
                    }
                }
              },params.row.BILLNO)
            ...
            switch (params.row.STATUS) {
              ...
              case "8":
                return h(
                  ...
                  {
                    attrs: {
                      title: params.row.REJECTREASON
                        ? "拒绝理由：" + params.row.REJECTREASON
                        : "拒绝理由:无"
                    }
                  },
                  "退单可重报"
  ...
  methods: {
    ...
    query() { 
      ...
      this.form.agentname = this.agentname;
      this.queryList();
    },
    queryList() {
      var url = "";
    ...
    exportFun() {
      // var
      publicInter(interfaceUrl.exportExhibitsForExcel, this.BillNoArr).then(
        r => {
          if (r.code === "500" || r.code === "400") {
            this.$Message.error(r.msg);
          } else {
            filedownload(
              interfaceUrl.exportExhibitsForExcel,
              this.BillNoArr
            ).then(r => {
    ...
    printTaste(obj) {
      ...
      publicInter(
        interfaceUrl.queryMaterialsExaminePrintByCertificateno,
        param
      ).then(r => {
        if (r) {
          if (r.code === "200") {
