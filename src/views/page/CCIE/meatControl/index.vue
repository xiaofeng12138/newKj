<template>
    <div>
        ...
        <Row style="text-align:left;margin-bottom:10px" class="searchRow">
            <span class="labelText">展商名称:</span>
            <Input v-model="params.name" class="customInput" size="large" style="width:150px;margin-right:12px;" />
            <span class="labelText">展台号:</span>
            <Input v-model="params.position" class="customInput" size="large" style="width:150px;margin-right:12px;"  @on-blur='blurBoothno' />
            <span>原产地:</span>
            <Input v-model="params.original" class="customInput" size="large" style="width:150px;margin-right:12px;" />      
            <Button @click="query('')" size='large' type="primary">查询</Button>
        </Row>  
        <Table :columns="columnsList" :data="dataList" class="customTable"></Table> 
        <Page show-total :total="totalNumber" v-if="totalNumber>0" :current="params.currPage" @on-change="changePage"  class="customPage"></Page>   
