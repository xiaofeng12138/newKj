...
export default {
    ...
    mounted(){
        this.radius = document.getElementById('rotate').offsetWidth/10;
    ...
    methods:{
        showEdit(params){
            // if(params.color == "#FFE91A"){
                this.$emit('showEdit',params, this.titles.GOODs);
            // }
        },
            ...
            publicInter(interfaceUrl.queryGoodsByExhType,params).then(r=>{
                ...
                if(r){
                    if(r.isOk || r.isOk == 'true'){
                        // "#FFE91A"黄色
                        var colors = ["#43C5FF","#8FA1FF","#FFFFFF","#FF9A55","#FF7676","#FF9A55","#FFFFFF","#5FB3FF","#8493EC","#FFE91A"];
                        this.titles.GOODs = [];
                        ...
                        for(let i = 0; i < r.lowGoods.length;i++){
                            ...
                            if(r.lowGoods[i].TOTALPRICE == -1){
                                unit.color = colors[i%10];
                                unit.value = r.lowGoods[i].GOODSDESCRIPTIONCN;
                                unit.EXHTYPE = r.lowGoods[i].EXHTYPE;
                                unit.UUID = r.lowGoods[i].UUID;
                                unit.IMAGEPATH = r.lowGoods[i].IMAGEPATH;
                                unit.GOODSDESCRIPTIONCN = r.lowGoods[i].GOODSDESCRIPTIONCN;
                                unit.GOODSDESCRIPTION = r.lowGoods[i].GOODSDESCRIPTION;
                                unit.POSITION = r.lowGoods[i].POSITION;
                                unit.show = true;
                                this.titles.GOODs.push(unit);
                            }
        ...
        update(){
            ...
            let sa = Math.sin(a * this.dtr),
                ...
                sb = Math.sin(0 * this.dtr),
                cb = Math.cos(0 * this.dtr),
        ...
        positionAll(){
            ...
            for (let i = 1; i < max+1; i++){
                ...
                this.mcList[i - 1].cx = 3*this.radius *Math.cos(theta)*Math.sin(phi);
                ...
                console.log(this.aSpan[i-1].style.left, this.aSpan[i-1].style.top)
            } 
        },
        // mouseoverFun(){
        //     clearInterval(this.timer);
