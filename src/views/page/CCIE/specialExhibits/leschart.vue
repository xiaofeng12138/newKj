<template>
    <div  :id="id" :paramKey="paramKey" :style="{width:width,height:height}"></div>
</template>
<script>
import interfaceUrl from '@/api/interfaceUrl'
import { publicInter } from '@/api/http'
export default {
    props:['width','height','num','id', 'paramKey'],
    data(){
        return {
            lesCharts:null,
            goodsList: []
    ...
    methods:{
        initLes(){
            let _this = this;
            this.$nextTick(() => {
                this.lesCharts = this.$echarts.init(document.getElementById(this.id));
                ...
                if(params.seriesType === 'graph' && params.dataType === 'node'){
                    if(params.value.includes(101.1)){
                        _this.$emit('showEdit',params.data, _this.goodsList);
                    }
                    
                }
            });
            this.tabnew(this.paramKey,1);
            })
        ...
        initGraph(valueArr){  
            ...
            let nodes = valueArr.lowGoods.sort(function(){return 0.5 - Math.random()});
            ...
            let links = [];
            // '#FFE91A'
            let color = ['#FFE91A', '#3CCE5A','#F9CEBB',,'#23B5EA','#4374FF','#23B5EA','#F9CEBB','#3CCE5A', ];
            for (let i = 0; i <nodes.length;i++){
                ...
                if(nodes[i].value == '101.1') {
                    nodes[i].show = true;
                    this.goodsList.push(nodes[i])
                }
                
                nodes[i].value +='万美元'
                nodes[i].attributes = {};
                nodes[i].attributes.modularity_class = Math.floor(Math.random()*8);
                nodes[i].itemStyle = {
                    normal:{
                        color:color[nodes[i].attributes.modularity_class]
                    }
                };
