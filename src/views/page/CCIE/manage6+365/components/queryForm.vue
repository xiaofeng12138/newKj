<template>
    <Row :gutter="16" style='margin-bottom:20px'> 
        <Col :span=6>
            <DatePicker size="large" type="date" v-model='time' placeholder="请选择日期" style="width:100%" class="customDatePicker"></DatePicker>
        </Col>
        <Col :span=4>
            <Select v-model="rangeTime" style="width:100%;margin-top:1.5px" class="customSelect" size="large">
                <Option v-for="item in rangeTimeOptions" :value="item.value" :key="item.value">{{ item.label }}</Option>
            </Select>
        </Col>
        <Col :span=3>
            <Select v-model="expoStatus" style="width:100%;margin-top:1.5px" class="customSelect" size="large" placeholder='选择身份'>
                <Option v-for="item in roleOptions" :value="item.value" :key="item.value">{{ item.label }}</Option>
            </Select>
        </Col>
        <Col :span=5>
             <Input v-model="form.variable" size="large" :placeholder="placeholder" class="customInput" clearable style="width: 100%"></Input>
        </Col>    
        <Col :span=4>
            <Input v-model="form.contractNo" size="large" placeholder="请输入合同编号" class="customInput" clearable style="width: 100%"></Input>
        </Col>
        <Col :span=2>
