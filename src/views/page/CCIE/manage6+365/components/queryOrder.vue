<template>
    <div>
        <query-form placeholder='请输入订单号' @queryEvents='query' />
        <Table stripe :columns="columns" :data="data" class="customTable" @on-selection-change='selectionChange'></Table>
        <div class="">
          ...
          :page-size='20'
           @on-change='pageChange'
            show-total class="customPage"></Page>
        </div>
...
export default {
                        ...
                        align: 'center',
                         render: (h, params) => {
                            ...
                            return h('div', [
                                ...
                                //            if(text==='确定'||text==='ok'){
                                //                that.confirm(params.row.PURCHASEORDERNO,params.index)
                                //            }
                                //        }
                                //     }
                                // },[
    ...
    methods:{
         query(val){
            ...
            this.queryParmas.purchaseOrderNO=this.queryParmas.variable
        ...
        switchUrl(){
            ...
            switch(role){
                case 'EA': 
                    this.url={
                        list:interfaceUrl.adminOrderList,
                        detail:interfaceUrl.adminOrderDetail,
                        del:''};
                         var option={
                            title: '操作',
                            key: 'action',
                ...
                case 'EI': 
                    this.isEI=true
                    this.url={
                        list:interfaceUrl.EIqueryOrderList,
                        detail:interfaceUrl.EIqueryOrderDetail,
                ...
                case 'EO': 
                    this.isEO=true
                    this.url={
                        list:interfaceUrl.EOqueryOrderList,
                        detail:interfaceUrl.EOqueryOrderDetail,
                        del:''};
        ...
        cancel(id,index){
            publicInter(this.url.del,{purchaseOrderNO:id}).then(r=>{
                if(r&&r.code==='200'){
        ...
        confirm(id,index){
            var arr=[].concat(id)
             publicInter(interfaceUrl.verifyExpoOrder,{purchaseOrderNO:id}).then(r=>{
                if(r&&r.code==='200'){
                    this.data.splice(index,1)
                    this.totalCount--
                }else{
        ...
        selectionChange(option){
            if(option){
                this.selectArr=[]
                option.forEach(val=>{
                    this.selectArr.push(val.PURCHASEORDERNO)
                })
            }
        },
        getdetail(id){
           publicInter(this.url.detail,{purchaseOrderNO:id}).then(r=>{
                if(r){
