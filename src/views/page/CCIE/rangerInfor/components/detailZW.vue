<template>
  <div class ='tt'>
        <div id="zw">
            <embed id="map" :src="require(`@/assets/svg/${this.numSite}.svg`)" />
             <!-- <embed id="map" :src="require(`@/assets/svg/MH.svg`)" /> -->
            <!-- <embed id="map" :src="require(`../../../../../assets/svg/${this.numSite}.svg`)" /> -->
            <!-- <embed id="map" :src="require('./newSvg/.svg')" /> -->
        </div>
      <div class="modal" v-show="modalShow" ref="modal">
          <div class="header">
              <Icon type="md-pin"></Icon>
              <span>{{exhibitionNum}}</span>
              <span class="closeBtn" @click="closeBtn">×</span>
          </div>
          <Table :columns="columns" :data="zwData" class="customTable" style="width: 100%" max-height="400"> </Table>
      </div>
      <div class="gwTooltip" ref="gwTooltip" v-show="gwTooltipShow">{{gwMsg}}</div>
      <Modal v-model="detailZWModalShow" :footer-hide="hide" width="1500" class="detailZWModal">
      ...
      <div class="goodsDetail" v-if="imgShow">
        ...
        <img :class="{'main-img':true,'my-show-img':true}" ref="goodImg" :src="`/exhibitorImage${this.imgs[this.imgIndex]}`"> 
...
export default {
    ...
    data() {
        return {
            imgUrl:require('./img/ranger1.png'),
            dataUrl:require(`@/assets/svg/${this.numSite}.svg`),
            nowYears:getCookie('date'),
            problemsValue: ['正常', '异常'],
            childIndexValue: 0,
            exhibitionNum: '',
            xgModalShow: false,
            ...
            columns: [
                {
                    ...
                    render: (h, params) => {
                        return h('span', {
                            style: {
                                //  color: params.row.GWSTATUS == 1 ? 'red' : '#ffc83e',
    ...
    watch: {
        numSite: {
            handler(newVal, oldVal) {
                ...
                let embed = `<embed id="map" src="${newVal}.svg" />`;
                // let embed = `<embed id="map" src=" @/assets/svg/${this.numSite}.svg`;
                // console.log('embed',embed)
                document.getElementById('zw').innerHTML = embed
                this.init()
            },
            deep: true
        }
    ...
    methods: {
        ...
        init() {
            let map = document.querySelector('embed');
            map.onload = () => {
                let d = map.getSVGDocument();
                let svg = d.querySelector('svg');
                let exhibit = svg.getElementsByClassName('exhibitionNum');
                for(let i = 0; i < exhibit.length; i++) {              
                    exhibit[i].addEventListener('click', (event) => {
                        this.$router.push({name:'zwlist',params:{
                            zwNum:exhibit[i].attributes.id.value
                        }})
                        // this.pisitionClick(event, exhibit[i].attributes.id.value)
                    });
                    exhibit[i].addEventListener('mouseover', (event) => {
                        exhibit[i].style.fill = '#00c1ff';
                    })
                    exhibit[i].addEventListener('mouseout', () => {
                        exhibit[i].style.fill = '';
                        this.gwTooltipShow = false;
                    })
                }
                publicInter(interfaceUrl.queryAreaListByHall, {hallno: String(this.numSite)}).then(r => {
                  
                    for(let i = 0; i < r.list.length; i++) {
                        let dom = svg.getElementById(r.list[i].BOOTHAREA);
                        // console.log(svg.getElementById(r.list[i].BOOTHAREA))
                        //巡馆状态
                        if(r.list[i].STATUS  == 1) {
                            //let doc_text = svg.getElementsByClassName(r.list[i].BOOTHAREA);
                            let doc_text = svg.getElementById(r.list[i].BOOTHAREA);
                          
                            let xmlns = "http://www.w3.org/2000/svg";
                            let svg_img = document.createElementNS(xmlns, "image");
                            svg_img.href.baseVal = this.imgUrl;

                            let text_size = doc_text.getBBox();
                            //  console.log(this.imgRightList.indexOf(r.list[i].BOOTHAREA))
                            // if(this.imgRightList.indexOf(r.list[i].BOOTHAREA) > -1) {//图放右边
                            //     svg_img.setAttributeNS(null, "x", text_size.x + text_size.width);
                            //     svg_img.setAttributeNS(null, "y", text_size.y + text_size.height/2-6);
                            // } else {//图放下面
                            svg_img.setAttributeNS(null, "x", text_size.x + text_size.width/2-6);
                            svg_img.setAttributeNS(null, "y", text_size.y + text_size.height/2-20);
                           
                            svg_img.setAttributeNS(null, "height", "15px");
                            svg_img.setAttributeNS(null, "width", "15px");
                            this.insertAfter(svg_img, doc_text);
                        }
                         //冷链
                        if(r.list[i].COLDSTATUS == 1 ) {
                            for(let index in dom.children) {
                                // console.log(dom)
                                if(dom.children[index].tagName == 'rect' || dom.children[index].tagName == 'path') {
                                    dom.children[index].style.fill = 'red';
                                    dom.children[index].style.stroke = 'red';
                                }
                            }
                        }
                        //高风险
                        if(r.list[i].GWSTATUS == 1 && r.list[i].COLDSTATUS == 0 ) {
                            for(let index in dom.children) {
                               if(dom.children[index].tagName == 'rect' ||  dom.children[index].tagName == 'path') {
                                    dom.children[index].style.fill = '#FF7521';
                                    dom.children[index].style.stroke = '#FF7521';
                                    dom.children[index].addEventListener('mouseover', (event) => {
                                        this.$refs.gwTooltip.style.left = event.pageX + 200  + 'px' ;
                                        this.$refs.gwTooltip.style.top = event.screenY - 100  + 'px' ;
                                         this.$refs.gwTooltip.style.backgroundColor =  '#FF7521';
                                        switch(r.list[i].GWTYPE) {
                                            case 0: 
                                                    this.gwMsg = '无高危'
                                                    break;
                                            case 1: 
                                            ...
                                            case 2: 
                                                    ...
                                                    break;
                                            case 3: 
                                                    this.gwMsg = '涉危化学品,曾被查获'
                                                    break;
                                        }
                                        this.gwTooltipShow = true;
                                    })
                                }
                            }
                        }
                        //重点关注
                        if(r.list[i].GFXSTATUS == 1 && r.list[i].COLDSTATUS  == 0 && r.list[i].GWSTATUS == 0 ) {
                           // let dom = svg.getElementById(r.list[i].BOOTHAREA);
                            ...
                            for(let index in dom.children) {
                                if(dom.children[index].tagName == 'rect' || dom.children[index].tagName == 'path') {
                                    dom.children[index].style.fill = 'yellow';
                                    dom.children[index].style.stroke = 'yellow';
                                }
                            }
                        }
                        //暂进展位
                        if( r.list[i].GWSTATUS == 0 && r.list[i].COLDSTATUS  == 0 && r.list[i].GFXSTATUS == 0 && r.list[i].ENTRYSTATUS == 1 ) {
                        setTimeout(()=>{
                        if(dom)
                           console.log('dom',dom.children)
                           if(dom.children){
                            for(let index in dom.children) {
                                if(dom.children[index]){
                                    if(dom.children[index].tagName == 'rect' || dom.children[index].tagName == 'path') {
                                    dom.children[index].style.fill = '#6BFF29';
                                    dom.children[index].style.stroke = '#6BFF29';
                                    console.log(1)
                                }
                                }
        ...
        pisitionClick(event, exhibitionNum) {
            let e = exhibitionNum;
            publicInter(interfaceUrl.queryBoothListByArea, {bootharea: exhibitionNum}).then(r => {
            ...
            this.$refs.modal.style.left = event.screenX > 1000 ? event.screenX - 720 + 'px' : event.screenX + 'px';
            this.$refs.modal.style.top = event.screenY >700 ? event.screenY - 600 + 'px' : event.screenY - 300 + 'px';
