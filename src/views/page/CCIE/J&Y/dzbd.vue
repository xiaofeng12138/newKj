<template>
    <div>
        <div class="title" v-if="showPage">
            <span>太平洋保险担保总额度/已用额度/货值：1(亿元)/{{(this.BOC.USEDAMOUNT / 10000).toFixed(0)}}(万元)/{{this.BOC.USDTOTAL}}(美元)</span>
        </div>
         <Table  border :columns = 'columns' :data='showdbTable' class="customTable" style="margin-top:20px" @on-row-click='rowClick'></Table>
            <Page
                    :total="total"
                    v-show="total"
                    show-total
...
export default {
    ...
    methods: {
       ...
       queryData(page){
        ...
        publicInter(interfaceUrl.queryGuranteeEntryList,requsetData).then(res=>{
            if(res.total>0){
