...
export default {
  ...
  computed: {
    ...mapState('bill', {
      ...
      mergeResultList: state => state.mergeResultList,
        preMergeListDisable:state => state.preMergeListDisable == 'false'?true:false
    })
  },
  methods: {
    ...mapActions('bill', [
    ...
    async next () {
         let param=this.$route.params.air=='yes'?{ billNo: this.billNo,air:'yes' }:{ billNo: this.billNo};
  ...
  console.log(this.$route.params,'uuuuuuuu')
    let param=this.$route.params.air=='yes'?{ air:'yes',billNo: this.billNo }:{billNo: this.billNo}
