<template lang="html">
  <div class="">
      <h1 style="padding-bottom: 20px;">空运提单订单物料关联</h1>
      <Row class="row-select" type="flex" align="middle">
        <Col span="5">
           <Input v-model="LINKID">
               <span slot='prepend'>总运单号</span>
           </Input>
        </Col>
        <Col span="5" style="margin-left:10px;">
           <Input v-model="HWBSERIALNUMBER">
               <span slot='prepend'>分运单号</span>
           </Input>
        </Col>
                  ...
                  if (params.row._status === '2') { // 未关联
                        ...
                        h('Button', {
                            ...
                            on: {
                                click: () => {
                                  // console.log(params);
                                   // this.myModel1 = true;
                                    this.BILL_NO = params['row']['BILLNO'];
                    ...
                    return h('div', {style:{'text-align':'left'}},[
                        ...
                        h('Button', {
                            ...
                            on: {
                                async click () {
                                  let r = await _this.delOrderYGL({ BILLNO: params['row']['BILLNO'],air:'yes' })
    ...
    async searchTidan () {
      let status = (this.status === '已关联') ? '1' : '2',
          param = {
            ...
            status,
            air:'yes',
            zongBillNo:this.LINKID,
            fenBillNo:this.HWBSERIALNUMBER,
      ...
      dingdan(dingdanStatus){
          let req = this.getOrderList({billNo: this.BILL_NO,dingDan:dingdanStatus,air:'yes'});
          req.then(r=>{
              if(r && r['code']=='500' ){
                  if(dingdanStatus === "no"){
                      let msg = "";
