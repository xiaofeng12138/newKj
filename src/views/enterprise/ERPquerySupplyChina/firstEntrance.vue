...
<div>
         <Table border :columns = 'columns1' :data='data1' class="self">
            <template  slot-scope="{row,index}" slot="action"> 
              ...
              <Modal v-model='modal1' width='600' :mask='false'  :scrollable='true' :styles="{top:'0px'}">
              <Form  ref='formData' :model="formData" :rules='formRules' :label-width ='180' style="margin-top:20px">
                ...
                <FormItem label='收货单位名称：' prop='PURCHASERNAME'> 
                 ...
                 <FormItem label='保税仓储企业名称：' prop='BONDEDWAREHOUSE'> 
                  <Input v-model="formData.BONDEDWAREHOUSE" style="width:80%;"/>
                </FormItem>
                <FormItem label='保税仓储企业统一信用代码：' prop='BWAREHOUSESOCIALCREDITCODE'> 
                  <Input v-model="formData.BWAREHOUSESOCIALCREDITCODE" style="width:80%;"/>
                </FormItem>
      ...
      return{
        //一线进口
         formData:{
              ...
              BWAREHOUSESOCIALCREDITCODE:''//仓储代码
         },
    ...
    submitone(name){
      this.$refs[name].validate((valid)=>{
        if(valid){
         ...
         let data = {
          ...
          BONDEDWAREHOUSE:this.formData.BONDEDWAREHOUSE,//保税仓储名称
          BONDEDWAREHOUSESOCIALCREDIT:this.formData.BWAREHOUSESOCIALCREDITCODE,//仓储代码
          UUID:this.num1
       }
       publicInter(interfaceUrl.updateGeneralMessage,data).then(r=>{
         console.log(r)
          if(r.msg.length > 7){
     ...
     //显示第一个
      showone(index){
        ...
        this.formData.BONDEDWAREHOUSE =this.data1[index].BONDEDWAREHOUSE
        this.formData.BWAREHOUSESOCIALCREDITCODE =this.data1[index].BONDEDWAREHOUSESOCIALCREDIT
      },
      delone(index){
        console.log(index)
       this.$Modal.confirm({
        ...
        publicInter(interfaceUrl.delGeneralMessage,data).then(r=>{
      ...
      queryInfoFirst(){
         ...
         publicInter(interfaceUrl.queryGeneralMessage,data).then(r=>{
           console.log(r)
            if(r.totalRow > 0){
              this.data1 = r.list
