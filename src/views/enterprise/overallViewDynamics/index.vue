...
// import {queryline} from '../../api/shipLine'
// require('../../vendor/world')

let echarts = require('echarts/lib/echarts.js')
// require('echarts/dist/echarts.js.map')
export default {
    created(){
         this.setWidthHeight()
    },
    ...
    methods:{
        ...
        concatEvent(arr){
            for(let i=0;i<arr.length;i++){
                var item=arr[i]
                if(i>=1&&item.jindu===arr[i-1].jindu&&item.weidu===arr[i-1].weidu){
            ...
            publicInter(interfaceUrl.qureyShipMsg,this.queryParams).then(r=>{
                if(r.isOk){
                    ...
                    r.msg.forEach((item,i)=>{
                        ...
                        item.pol_pod.forEach(val=>{   // 收集所有坐标点
                   ...
                   let flag=this.iscrossEW()
                    ...
                    if(flag){
                        ...
                        var len1=segment1.length,len2=segment2.length
                        options.series[3]['data']=[{coords:segment1.reverse()},{coords:segment2.reverse()}]
                        options.series[2]['data']=[{coords:segment1},{coords:segment2}]
                        
                    }else{
                        options.series[3]['data']=[{coords:this.dataMsg.coordinate}]
                         options.series[2]['data'][0]={coords:this.dataMsg.coordinate.reverse()}
                    }
                    ...
                    this.dataMsg.eventsName.forEach((val,i)=>{
                        ...
                        this.eventsArray.push({name:arr2,active:i===0,time:arr1})
                    })
                    this.charts.setOption(options,true)
                }else{
        ...
        changeOption(index){
            ...
            if(index===0){
                options.series[2]['data'][0]['coords']=dataline.slice(endIndex).concat([this.dataMsg.coordinate[this.dataMsg.coordinate.length-1]])
            }else{
                options.series[2]['data'][0]['coords']=dataline.slice(endIndex).reverse()
            }
            this.charts.setOption(options,true)
        },
...
<style rel='stylesheet/scss' lang="scss" scoped>
    .overView{
        ...
        .events-swepper{
            ...
            padding: 30px 15px;
            background: #1A3A84;
            box-shadow: 0 2px 12px 0 rgba(12,22,47,0.30);
            .box{
                height: 85%;
                width: 100%;
                margin-top: 15%;
