<template lang="html">
    <div class="">
        <h1 style="padding-bottom: 20px;">报关基本信息查询</h1>
        <Row class="searchRow">
            <Col span="6">
    ...
    export default {
                                ...
                                h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                           ...
                                           if(this.$route.name=='baoguanSearchair'){
                                                this.getViewModalData({billNo:params.row.BILLNOP,CNCOMPANYCODE:CNCOMPANYCODE,url:interfaceUrl.seeBlAlLinkPOListEP,air:'yes'})
                                           }else{
                                               this.getViewModalData({billNo:params.row.BILLNOP,CNCOMPANYCODE:CNCOMPANYCODE,url:interfaceUrl.seeBlAlLinkPOListEP})
                                           }
                                            this.viewModal = true;
                                        }
                                    }
        ...
        methods: {
            ...
            getViewModalDataFun(){
                ...
                let url = (this.role.indexOf('CB')!="-1")?interfaceUrl.seeBlAlLinkPOListCB:interfaceUrl.seeBlAlLinkPOListEP;
                if(this.billNo && this.$route.params.CNCOMPANYCODE){
                    ...
                    if(this.$route.params.erpTempnum){
                        url = interfaceUrl.seeBlAlLinkPOListAfChaiDan2;
                        ...
                        this.getViewModalData({billNo:this.billNo,transmode:this.transmode, CNCOMPANYCODE:this.CNCOMPANYCODE,url:url})
                    }
                    this.viewModal = true
                }
                else if(this.billNo){
                    if(this.$route.params.air=='yes'){
                         this.getViewModalData({billNo:this.billNo,url:url,transmode:this.transmode,air:'yes'})
                    }else{
                          this.getViewModalData({billNo:this.billNo,url:url,transmode:this.transmode})
                    }
                    this.viewModal = true
                }
            },
            ...
            search () {
                if(this.$route.name=='baoguanSearchair'){
                    this.billPreview({ billNo: this.inputValue, date: this.selectDateObj,role:this.role,air:'yes' });
                }else{
            ...
            async ok (clickType,edi='false') {
                ...
                let url = this.role.indexOf('EP')<0?'/cb/bLAuthoCusBrok/downloadXmlFileByBLNo':"/ep/bLLinkWithOrder/downloadXmlFileByBLNo";
                if(this.$route.params.erpTempnum){
                    
                    url = "/cb"+interfaceUrl.downloadXmlFileByBLNoAfChaiDan2;
                }
                console.log(url)
                let res = await this.xmlDownload({transmode:this.transmode,billNo: this.billNo,clickType,CNCOMPANYCODE:this.CNCOMPANYCODE,url:url,edi,id:this.$route.params.erpTempnum })
                // console.log(res);
                this.$Spin.hide()
        ...
        mounted () {
            ...
            console.log(this.codeData,'iiiii');
        }
    }
</script>
